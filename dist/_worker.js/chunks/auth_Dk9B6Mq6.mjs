...
const JWT_SECRET = new TextEncoder().encode(
  "vkrama-dev-secret-change-in-production"
);
async function createToken(payload, expiresInHours) {
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setExpirationTime(`${expiresInHours}h`).setJti(payload.sessionId).sign(JWT_SECRET);
}
...
function getCustomerSessionExpiry() {
  const date = /* @__PURE__ */ new Date();
  date.setDate(date.getDate() + SESSION_EXPIRY_DAYS);
  return date.toISOString();
}
async function hashPassword(password) {
  const { bcrypt } = await import('./index.esm_CjyeKsOt.mjs');
...
export { verifyPassword as a, getAdminSessionExpiry as b, createToken as c, getCustomerSessionExpiry as d, generateId as g, hashPassword as h, nanoid as n, verifyToken as v };
