...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    let subtotalCents = 0
    const lineItems: Array<{ name: string; quantity: number; priceCents: number; imageUrl?: string; slug: string; productId: string; variantId?: string; variantName?: string }> = []
    ...
    const shippingCents = shippingInfo ? (subtotalCents >= 5000 ? 0 : 599) : 0
